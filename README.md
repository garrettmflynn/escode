...
**escode** implements the [ES Components] specification, which allows you to define special properties on a hierarchy of reactive objects.
...
## ES Properties
ES properties refer to **special properties that are used to instantiate the Component**. These properties are prefixed with `__` and are recognized with **loaders** than can be used to experiment with new Component behaviors

> See the [ES Components] specification for a full list of default properties. 

All ES Components have at least one ES property at instantiation. All other properties throughout an ES Component are listeneable by the root Component.
...
All other `__` properties are considered ES properties, and are used to program the behavior of the Component. 
...
In the near future, we will switch to the registration of ES Components through NPM via standardized use of the `escomponent` keyword. These existing components will be published and distributed into independent repositories.
