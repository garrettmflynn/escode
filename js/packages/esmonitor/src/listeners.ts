...
export const info = (id, callback, path, originalValue, base, listeners, options: MonitorOptions, refShortcut: Partial<ReferenceShortcut> = {}) => {
    ...
    const info = {
        ...
        // Note: this doesn't need to be dynamic if we handle all the unsubscriptions externally, which we should, based on reparenting...
        // get parent() { 
        //     return get(shortcutPath ? shortcutPath?.slice(0,-1) : info.path.parent) 
        // },
        parent: get(shortcutPath ? shortcutPath?.slice(0,-1) : completePathInfo.parent) ,
...
const registerInLookup = (id, name, sub, lookups) => {
...
export const register = (info, collection, lookups?: ListenerLookups) => {
    ...
    const relative = utils.getPath('relative', info)
    if (!collection[info.id]) collection[info.id] = {}
    const thisCollection = collection[info.id]

    if (!thisCollection[relative]) thisCollection[relative] = {}
    thisCollection[relative][info.sub] = info

    // Place in Lookup Registry
    registerInLookup(info.id, relative, info.sub, lookups)
    return true
}
...
export const set = (type, absPath, value, callback, base, allListeners: Partial<ListenerRegistry>, options: MonitorOptions) => {
    ...
    if (listeners[type]) listeners[type](fullInfo, allListeners, allListeners.lookup) // goes up to register()
        ...
        if (!allListeners[type][id]) allListeners[type][id] = {}
        const thisListeners = allListeners[type][id]
        thisListeners[fullInfo.path.relative.join(options.keySeparator)][fullInfo.sub] = fullInfo
        if (allListeners.lookup) registerInLookup(id, path, fullInfo.sub, allListeners.lookup)
    }
}
...
export const setterExecution = (listeners, value) => {
    return utils.iterateSymbols(listeners, (_, o: ListenerInfo) => {
        ...
        utils.runCallback(o.callback, path,  {id: o.id}, value)
    })
}
                ...
                try {
                    Object.defineProperty(parent, info.last, {
                        ...
                        set: async (v) => {
                            ...
                            val = v
                            if (!isFunction) {
                                const listeners = Object.assign({}, collection['setters'][info.id][utils.getPath('relative', info)])
...
export function getProxyFunction(info, collection, fn) {
    return function (...args) {
        const listeners = collection['functions'][info.id][utils.getPath('relative', info)]
...
export const functionExecution = (context, listeners, func, args) => {
    ...
    utils.iterateSymbols(listeners, (_, o: ListenerInfo) => {
        const path = utils.getPath('output', o)
        const info = executionInfo.value
        info.id = o.id
        utils.runCallback(o.callback, path, info, executionInfo.output)
    })
