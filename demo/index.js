(() => {
  ...
  function set(text = this.nExecutions) {
      ...
      this.run = (...args) => {
        ...
        return new Promise(async (resolve) => {
          if (this) {
            ...
            let runnode = async () => {
              let res = await run2(this, void 0, ...args);
              if (res !== void 0) {
...
"${n2.tag}:${n2.print(n2, printChildren)}"`;
  ...
  var runInGraph = document.getElementById("runInGraph");
  var container = document.getElementById("container");
  var startExecution = async () => {
    const options = { parentNode: container };
    const instance = new src_default(example_exports, options);
    ...
    e2.nExecutions = -1;
    const secondInstance = new src_default(e2, options);
    await secondInstance.start();
    const apiInstance = new src_default(api_exports, options);
