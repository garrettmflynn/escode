...
export function esConnected () {
    ...
    window.addEventListener('beforeunload', async () => await this.stop.close())
}
...
export async function stop() {
    ...
    if (this.operator) {
        // await this.operator.execute("Stop");
        this.operator.close()
    }
...
export async function connect () {
    ...
    if (!this.status || this.status === 'disconnected' || this.status === 'failed') {
        ...
        this.status = 'active'
            ...
            console.warn('BCI2000 is already running')
        }
}