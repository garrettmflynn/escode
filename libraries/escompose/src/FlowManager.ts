...
class FlowManager {
    ...
    getManager = (mode ='from') => {
            ...
            return target.__toResolveWith ?? this
    }
    ...
    #getPathInfo = (path) => {
        ...
        const isComponent = obj?.hasOwnProperty('__isESComponent')

        // Updates based on default
        if (isComponent) {
            output.absolute.array.push(standards.defaultPath)
            output.relative.array.push(standards.defaultPath)
        }
    ...
    add = (from, to, value: any = true, subscription) => {
        ...
        if (!subscription) subscription = this.globals.active[absPath]?.__esSubscription
        ...
        refs.forEach(ref => {
            ...
            if (!base.__esSubscription) {
                Object.defineProperty(base, '__esSubscription', {
                    value: subscription,
                    configurable: true
                })
            }
    ...
    addToGlobalLog = (path, mode = 'from') => {
        ...
        globalPath.forEach((key) => {
            if (!target[key]) target[key] = {}
            target = target[key]
            if (!('__toResolveWith' in target)) target.__toResolveWith = this // Always set with the lowest
        })
...
pass = (from, target, update) => {
    ...
    if (
        ...
        // Direct Function
            ...
            if (noContext) target.call(config?.esBind?.value ?? this.context.instance, ...arrayUpdate) // Call with top-level context
