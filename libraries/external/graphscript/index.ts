...
export class Graph {
    ...
    subscribe = (
        ...
        if(typeof callback === 'string' && target) {
            ...
            // // console.log('I AM A STRING', method)

            let thisNode = this.get(target)
            // key = callback;
            callback = `${thisNode.__node.tag}.${callback}`;

            // if(typeof method === 'function') callback = (...args) => {
