...
import * as brainsatplay from '../drafts/esedit/src/index';
...
async function start(input, options=appOptions){
   ...
   const esc = await app.start(input, options).catch(e => console.error('Invalid App', e))

    if (esc) editor.setApp(app)
